...
function JoinUsForm() {
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        const response = await axios.post('http://localhost:5004/submit-form', formDataToSend);
        console.log( response.data);
        alert('We receive your message. We will reply to you soon');
      ...
      <form className="max-w-2xl mx-auto p-8 rounded-lg mt-24" onSubmit={formik.handleSubmit}>
        ...
        <div className="space-y-6">
          ...
          <div className="relative flex items-center">
           ...
           <button
          type="submit"
            className="bg-transparent text-black border border-black hover:bg-black hover:text-white hover:border-black focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium text-sm w-full sm:w-auto px-5 py-2 text-center"
            >
            Apply Now
         </button>
